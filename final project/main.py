...
def second_page():
    ...
    # Виджет ввода текста для URL изображения
    ...
    # Обработка события нажатия на кнопку
    if st.button("Detect", key="recognition_button_detect"):
        # Если введен URL, загрузить изображение
        if image_url:
            try:
                ...
                # Обработка события нажатия на кнопку
                st.markdown("<p class='subtitle3'>Detected tourist attraction:</p>", unsafe_allow_html=True)
